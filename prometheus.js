...
client.on('message', message => {
	...
	if(message.content.toLowerCase().includes('just chris evans')) {
		...
		jsdom.env(
			...
			function (err, window) {
				...
				var randChris = window.$('body p:eq('+ randPic +') img').attr("src");
	...
	if(message.content.toLowerCase().includes('hc porn for martian')) {
		genURL = "https://cidsupplies.000webhostapp.com/fmbm.html";
